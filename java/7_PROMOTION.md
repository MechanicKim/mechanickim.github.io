...
- **정수보다 실수가 항상 큼** (비트 수가 같아도 실수는 지수 방식으로 표현하기 때문에 훨씬 큰 수를 담을 수 있습니다.)
- **char 타입**은 `int`로 자동 변환될 수 있지만, `byte`는 `char`로 자동 변환될 수 없다. (`byte`는 음수가 있고, `char`는 없기 때문)
...
float floatValue = longValue;   // 자동 타입 변환 (long -> float)
double doubleValue = longValue; // 자동 타입 변환 (long -> double)
```
...
```java
char charValue = 'A';
int intValue = charValue;   // 자동 타입 변환 ('A'의 유니코드 65 저장)
```
