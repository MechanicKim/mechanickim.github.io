...
### 2. 반대로: 기본 타입을 문자열로 변환

1. **`String.valueOf()`**: 가장 정석적인 방법

```java
String str = String.valueOf(100);
```

2. **문자열 결합(`+`)**: 빈 문자열과 더하면 자동으로 문자열이 됨

```
String str = 100 + "";
```

### 3. 주의사항: NumberFormatException

문자열을 숫자로 바꿀 때, 문자열에 숫자가 아닌 문자(알파벳, 특수문자 등)가 포함되어 있으면 `NumberFormatException`이`라는 에러가 발생하며 프로그램이 멈출 수 있다.
